...
"""
import numpy as np
from numerical.space.fd import FiniteDifference
from numerical.space.fftd import FFTDerivatives
from numerical.time.integration import Integration
...
class Fire:
    ...
    def solvePDE_high_precision(self, Nx, Ny, Nt, u0, b0, v, space_method='fd', time_method='RK4', last=True, **kwargs):
        """Solve numerical PDE.
        ...
        **kwargs : dict
        ...
        if space_method == 'FD': # Finite Differences
            ...
            U0 = np.asarray(U0, dtype=np.longdouble)
            B0 = np.asarray(B0, dtype=np.longdouble)
        ...
        # Vectorize variables for Method of Lines. [vec(U), vec(B)]^T
        y0 = np.zeros((2 * Ny * Nx), dtype=np.longdouble)
        y0[:Ny * Nx] = U0.flatten('F')
        y0[Ny * Nx:] = B0.flatten('F')
