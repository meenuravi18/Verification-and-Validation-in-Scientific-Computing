...
class FiniteDifference:
    ...
    def sourceTerms(self, t):
        """Source terms of PDE. Option 1 from Homework 3.
        ...
        Returns
        -------
        Su: array_like, shape (Ny, Nx)
            Source term for temperature variable.
        Sb: array_like, shape (Ny, Nx)
            Source term for fuel variable.

        """
        X, Y= self.getMesh()
        u_hat = (1+t)*np.sin(np.pi*X)*np.sin(np.pi*Y)
        beta_hat = (1+t)*np.sin(np.pi*X)*np.sin(np.pi*Y)
        Su =  np.sin(np.pi * X) * np.sin(np.pi * Y) + 2*self.kap * np.pi**2 * (1+t)*np.sin(np.pi * X) * np.sin(np.pi * Y)
        beta_t_hat = np.sin(np.pi*X)*np.sin(np.pi*Y)
        Sb = beta_t_hat - self.g(u_hat, beta_hat)
    ...
    def RHS(self, t, y):
        ...
        # Option 1 from Homework 3
        Su, Sb= self.sourceTerms(t)
        Uf += Su
        Bf += Sb
